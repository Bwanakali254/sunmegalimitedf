...
const SearchBar = () => {
    ...
    const visible = useMemo(() => {
        return location.pathname.includes('collection') && showSearch;
  ...
  return showSearch && visible ? (
    <div className='border-t border-b bg-gray-50 text-center'>
        ...
        <img src={assets.searchIcon} alt="search" className='w-4 cursor-pointer' />
        </div>
        <img src={assets.crossIcon} alt="Close" className='inline w-3 cursor-pointer' onClick={() => setShowSearch(false)} />
    </div>
  ) : null;
}
